...
try:
    ...
    print("⚠️  MinIO package not available - raw data storage disabled")
...
class FixedEnhancedBatchProcessor:
    ...
    def store_raw_data_minio(self, df, filename):
        ...
        if not MINIO_AVAILABLE or self.minio_client is None:
            print(f"  ⚠️  MinIO not available - skipping raw data storage: {filename}")
            return True
    ...
    def store_processed_data_postgres(self, df):
        ...
        try:
            ...
            if problematic_rows > 0:
                print(f"  ⚠️  Skipped {problematic_rows} problematic rows")
        ...
        try:
            ...
            for chunk_number, chunk in enumerate(pd.read_csv(file_path, chunksize=chunk_size), 1):
                if total_processed >= demo_limit:
                    print(f"  ⏸️  Demo limit reached ({demo_limit:,} records)")
                    break
                ...
                if success_count == 0:
                    print(f"  ⚠️  No records stored from chunk {chunk_number} - possible data quality issues")
