...
class ValidatedBatchProcessor:
    ...
    def _init_database(self):
        ...
        try:
            ...
            self.logger.warning(f"⚠️ DatabaseClient initialization failed: {e}")
    ...
    def save_to_database(self, transactions):
        ...
        if not self.db:
            self.logger.warning("⚠️ No database connection - using fallback")
        ...
        try:
            ...
            for i, transaction in enumerate(transactions):
                try:
                    ...
                    self.logger.warning(f"⚠️ Failed to save transaction {i}: {e}")
                    continue
    ...
    def _print_production_report(self, result):
        ...
        print(f"💾 Database: {'✅ CONNECTED' if result.get('database_used') else '⚠️ FALLBACK'}")
