...
try:
    ...
    print("⚠️ Monitoring not available - continuing without advanced metrics")

print("⚠️ Running without MinIO storage")
...
class FinalBatchProcessor:
    ...
    def process_large_dataset(self, total_records=1000000, chunk_size=100000, demo_limit=300000):
        ...
        for chunk_start in range(0, total_records, chunk_size):
            ...
            self.logger.info(f"   ⏱️  Times - Generation: {generation_time:.2f}s, Processing: {chunk_processing_time:.2f}s, Total: {chunk_total_time:.2f}s")
        ...
        if final_count > total_stored:
            ...
            self.logger.info(f"   ℹ️  Database has {previous_records:,} previous records from earlier runs")
            ...
            self.logger.info(f"   ⚠️  Verification status: UNEXPECTED MISMATCH")
        ...
        self.logger.info(f"   Monitoring enabled: {'✅ YES' if HAS_MONITORING else '⚠️ BASIC'}")
...
def main():
    ...
    try:
        ...
        if validation_results['valid_structure'] / validation_results['total_checked'] < 0.95:
            processor.logger.warning("⚠️  Data quality below 95% - proceeding with caution")
        else:
